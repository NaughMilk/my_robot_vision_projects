...
SIGN_LOCAL_MANEUVER_MAX_ROUTE_DISTANCE_M = 0.35
SIGN_LOCAL_MANEUVER_SYNC_SUPPRESS_S = 4.0
...
TRAFFIC_LIGHT_CAMERA_STATE_ASPECT_MAX = {
    "red": 3.5,
    "yellow": 2.8,
    "green": 4.0,
}
TRAFFIC_LIGHT_CAMERA_STATE_MIN_FILL_RATIO = {
    "red": 0.42,
    "yellow": 0.50,
    "green": 0.58,
}
TRAFFIC_LIGHT_CAMERA_STATE_MAX_AREA_RATIO = {
    "green": 0.0030,
}
TRAFFIC_LIGHT_CAMERA_STATE_MAX_CENTER_Y_RATIO = {
    "green": 0.68,
}
TRAFFIC_LIGHT_CAMERA_MAX_COMPONENT_WIDTH_RATIO = 0.14
TRAFFIC_LIGHT_CAMERA_MAX_COMPONENT_HEIGHT_RATIO = 0.16
...
CAMERA_SIGN_NON_PARKING_ACTION_MAX_DISTANCE_M = 0.95
CAMERA_SIGN_NON_PARKING_FACE_MIN_DOT = 0.70
...
TRAFFIC_LIGHT_SIGN_SUPPRESSION_SCALE = 2.8
TRAFFIC_LIGHT_SIGN_SUPPRESSION_MIN_PX = 26
CAMERA_SIGN_SPEED_LIMIT_SOLID_RED_FILL_MAX = 0.62
...
class DirectedTrack:
    ...
    def rejoin_nearby_maneuver(
        ...
        if max_route_distance_m is not None and projection.distance_m > max_route_distance_m:
            return None
        self.current_segment = projection.segment
        self.distance_on_segment = min(max(projection.distance_on_segment, 0.0), projection.segment.length)
        self.distance_since_branch = 0.0
    ...
    def _maneuver_actions_for_choices(self, segment: TrackSegment, choices: list[TrackSegment]) -> set[str]:
        current_yaw = segment.sample(max(0.0, segment.length - YAW_LOOKAHEAD_M))[2]
        actions: set[str] = set()
        for choice in choices:
            if self.segment_is_parking_choice(choice):
                continue
            sample_distance = min(choice.length, MANEUVER_LOOKAHEAD_M)
            candidate_yaw = choice.sample(sample_distance)[2]
            delta = normalize_angle(candidate_yaw - current_yaw)
    ...
    def maneuver_actions_at_next_branch(self, max_distance_m: float) -> set[str]:
        if self.stopped_at_terminal or self.current_segment.parking_route:
            return set()
        ...
        while distance_to_branch <= max_distance_m and segment.name not in visited:
            visited.add(segment.name)
            choices = self.outgoing.get(segment.end_node, [])
            if self._choices_have_controlled_branch(segment, choices):
                if self.distance_since_branch + distance_to_branch >= MIN_BRANCH_DISTANCE_M:
                    return self._maneuver_actions_for_choices(segment, choices)
                return set()
...
class GazeboTrackCar(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("sign_action_max_distance_m", CAMERA_SIGN_NON_PARKING_ACTION_MAX_DISTANCE_M)
        self.declare_parameter("parking_sign_action_max_distance_m", 1.35)
        ...
        self.declare_parameter("initial_pose_override_enabled", False)
        self.declare_parameter("initial_pose_x_m", 0.0)
        self.declare_parameter("initial_pose_y_m", 0.0)
        self.declare_parameter("initial_pose_yaw_rad", 0.0)
        ...
        requested_sign_action_max_distance_m = max(
            0.1,
            float(self.get_parameter("sign_action_max_distance_m").get_parameter_value().double_value),
        )
        self.sign_action_max_distance_m = min(
            requested_sign_action_max_distance_m,
            CAMERA_SIGN_NON_PARKING_ACTION_MAX_DISTANCE_M,
        )
        self.parking_sign_action_max_distance_m = max(
            self.sign_action_max_distance_m,
            float(self.get_parameter("parking_sign_action_max_distance_m").get_parameter_value().double_value),
        )
        ...
        self.initial_pose_override_enabled = bool(
            self.get_parameter("initial_pose_override_enabled").get_parameter_value().bool_value
        )
        self.initial_pose_x_m = float(self.get_parameter("initial_pose_x_m").get_parameter_value().double_value)
        self.initial_pose_y_m = float(self.get_parameter("initial_pose_y_m").get_parameter_value().double_value)
        self.initial_pose_yaw_rad = normalize_angle(
            float(self.get_parameter("initial_pose_yaw_rad").get_parameter_value().double_value)
        )
        ...
        if self.initial_pose_override_enabled:
            projection = self.track.nearest_pose_projection(
                self.initial_pose_x_m,
                self.initial_pose_y_m,
                normalize_angle(self.initial_pose_yaw_rad - self.yaw_offset),
                outer_only=False,
            )
            self.track.switch_to_projection(
                projection,
                f"route/initial_pose_override: {projection.segment.name}",
            )
        ...
        self.route_alignment_suppressed_until_ns = 0
        ...
        self.last_camera_sign_action_distance_guard_signature: tuple[str, str, str] | None = None
        self.last_camera_sign_light_guard_signature: tuple[str, str, str] | None = None
        self.logged_traffic_light_shape_guard_signatures: set[tuple[str, str]] = set()
        ...
        self.get_logger().info(
            ...
            f"action_dist<={self.sign_action_max_distance_m:.2f}m "
            f"parking_action_dist<={self.parking_sign_action_max_distance_m:.2f}m "
            ...
            f"override={self.initial_pose_override_enabled} "
    ...
    def publish_wheel_motion(self, stamp_ns: int) -> None:
        ...
        if (
            self.wheel_control_nearest_route_enabled
            and not self.track.stopped_at_terminal
            and stamp_ns >= self.route_alignment_suppressed_until_ns
        ):
    ...
    def detect_camera_traffic_light(self, frame: np.ndarray) -> TrafficLightCameraDetection | None:
        ...
        states_to_check = (
            (self.traffic_light_state,)
            if self.traffic_light_state in TRAFFIC_LIGHT_CAMERA_COLOR_RANGES
            else tuple(TRAFFIC_LIGHT_CAMERA_COLOR_RANGES)
        )
        for state in states_to_check:
            ranges = TRAFFIC_LIGHT_CAMERA_COLOR_RANGES[state]
            ...
            candidate = self.best_traffic_light_component(state, hsv, mask, roi_top, height, max_area)
    ...
    def best_traffic_light_component(
        ...
        for label_index in range(1, component_count):
            ...
            if not self.traffic_light_component_shape_allows(
                state,
                w,
                h,
                area,
    ...
    def traffic_light_component_shape_allows(
        self,
        state: str,
        width_px: int,
        height_px: int,
        ...
        max_aspect = TRAFFIC_LIGHT_CAMERA_STATE_ASPECT_MAX.get(state, 4.0)
        if aspect > max_aspect:
            self.log_traffic_light_shape_reject(state, "aspect", width_px, height_px)
            return False

        min_fill = max(
            self.traffic_light_camera_min_fill_ratio,
            TRAFFIC_LIGHT_CAMERA_STATE_MIN_FILL_RATIO.get(state, self.traffic_light_camera_min_fill_ratio),
        )
        if fill_ratio < min_fill:
            self.log_traffic_light_shape_reject(state, "fill", width_px, height_px)
            return False

        max_area_ratio = TRAFFIC_LIGHT_CAMERA_STATE_MAX_AREA_RATIO.get(state)
        if max_area_ratio is not None:
            max_area = max(self.traffic_light_camera_min_area_px, int(frame_width * roi_height * max_area_ratio))
            if area_px > max_area:
                self.log_traffic_light_shape_reject(state, "area", width_px, height_px)
                return False

        max_width = max(18, int(frame_width * TRAFFIC_LIGHT_CAMERA_MAX_COMPONENT_WIDTH_RATIO))
        max_height = max(18, int(roi_height * TRAFFIC_LIGHT_CAMERA_MAX_COMPONENT_HEIGHT_RATIO))
        if width_px > max_width or height_px > max_height:
            self.log_traffic_light_shape_reject(state, "large", width_px, height_px)
            return False

        max_center_y_ratio = TRAFFIC_LIGHT_CAMERA_STATE_MAX_CENTER_Y_RATIO.get(state)
        if max_center_y_ratio is not None and center_y_px > int(frame_height * max_center_y_ratio):
            self.log_traffic_light_shape_reject(state, "low", width_px, height_px)
            return False

        return True

    def log_traffic_light_shape_reject(
        self,
        state: str,
        reason: str,
        width_px: int,
        ...
        if state != "green":
            return
        if width_px < 80 and height_px < 24:
            return
        signature = (state, reason)
        if signature in self.logged_traffic_light_shape_guard_signatures:
            return
        self.logged_traffic_light_shape_guard_signatures.add(signature)
        self.get_logger().info(
            "camera traffic-light green candidate ignored as non-lamp-shaped color patch; "
            f"reason={reason} bbox={width_px}x{height_px}px"
        )

    def current_camera_traffic_light_bbox(
        self,
        frame_width: int,
        frame_height: int,
        stamp_ns: int,
        ...
        if not self.camera_traffic_light_is_current(stamp_ns):
            return None
        if self.camera_traffic_light_center is None or self.camera_traffic_light_size is None:
            return None

        center_x, center_y = self.camera_traffic_light_center
        width_px, height_px = self.camera_traffic_light_size
        half_width = max(float(width_px) * scale * 0.5, float(min_size_px) * 0.5)
        half_height = max(float(height_px) * scale * 0.5, float(min_size_px) * 0.5)
        x0 = max(0, int(math.floor(center_x - half_width)))
        y0 = max(0, int(math.floor(center_y - half_height)))
        x1 = min(frame_width, int(math.ceil(center_x + half_width)))
        y1 = min(frame_height, int(math.ceil(center_y + half_height)))
        if x1 <= x0 or y1 <= y0:
            return None
        return x0, y0, x1, y1

    def suppress_camera_traffic_light_from_sign_mask(
        self,
        ...
        bbox = self.current_camera_traffic_light_bbox(
            frame_width,
            frame_height,
            stamp_ns,
            TRAFFIC_LIGHT_SIGN_SUPPRESSION_SCALE,
            TRAFFIC_LIGHT_SIGN_SUPPRESSION_MIN_PX,
        )
        if bbox is None:
            return
        ...
        if x1 <= x0 or mask_y1 <= mask_y0:
            return
        color_mask[mask_y0:mask_y1, x0:x1] = 0

    def camera_sign_candidate_overlaps_current_traffic_light(
        self,
        ...
        light_bbox = self.current_camera_traffic_light_bbox(
            frame_width,
            frame_height,
            stamp_ns,
            TRAFFIC_LIGHT_SIGN_SUPPRESSION_SCALE,
        ...
        lx0, ly0, lx1, ly1 = light_bbox
        overlap_width = min(x1, lx1) - max(x0, lx0)
        overlap_height = min(y1, ly1) - max(y0, ly0)
        if overlap_width <= 0 or overlap_height <= 0:
            return False

        signature = (self.track.current_segment.name, self.camera_traffic_light_state or "-", "overlap")
        if signature != self.last_camera_sign_light_guard_signature:
            self.get_logger().info(
                "camera speed-limit candidate ignored because it overlaps the traffic-light bbox; "
                f"segment={self.track.current_segment.name} "
                f"light={self.camera_traffic_light_state or '-'} "
                f"candidate_bbox={x1 - x0}x{y1 - y0}px "
                f"light_bbox={lx1 - lx0}x{ly1 - ly0}px"
            )
            self.last_camera_sign_light_guard_signature = signature
        return True

    def camera_speed_limit_geometry_allows(
        self,
        red_component_fill_ratio: float,
        component_width_px: int,
        component_height_px: int,
    ) -> bool:
        if red_component_fill_ratio <= CAMERA_SIGN_SPEED_LIMIT_SOLID_RED_FILL_MAX:
            return True

        signature = (
            self.track.current_segment.name,
            "solid_red",
            f"{component_width_px}x{component_height_px}",
        )
        if signature != self.last_camera_sign_light_guard_signature:
            self.get_logger().info(
                "camera speed-limit candidate ignored as solid red lamp-like blob; "
                f"segment={self.track.current_segment.name} "
                f"component_bbox={component_width_px}x{component_height_px}px "
                f"red_fill={red_component_fill_ratio:.2f}"
            )
            self.last_camera_sign_light_guard_signature = signature
        return False
    ...
    def refresh_speed_limit_release(self) -> None:
        ...
        cleared_name = self.active_speed_limit_name
        self.get_logger().info(
            f"camera speed limit sign {cleared_name} cleared; "
            f"speed restored to {self.speed:.2f}m/s "
            f"segment={self.track.current_segment.name} "
            f"distance={self.track.distance_on_segment:.3f}m"
        )
        if cleared_name is not None:
            self.applied_speed_limit_names.discard(cleared_name)
    ...
    def update_latched_traffic_light_stop(self, stamp_ns: int) -> bool:
        ...
        if (
            ...
            and (not camera_visible or self.camera_traffic_light_state == "green")
        ):
            self.release_traffic_light_stop_latch(stamp_ns, "cycle_green_after_min_stop_camera_not_red")
            return False
    ...
    def detect_camera_instruction(
        self,
        frame: np.ndarray,
        stamp_ns: int,
    ) -> CameraInstructionDetection | None:
        ...
        self.suppress_camera_traffic_light_from_sign_mask(
            color_mask,
            roi_top,
            frame_width,
            frame_height,
        ...
        for label_index in range(1, component_count):
            ...
            if max(w / h, h / w) > 5.0:
                continue
            component_fill_ratio = colored_area / float(max(1, w * h))
            ...
            if template.kind == "speed_limit_sign":
                frame_bbox = (x0, y0 + roi_top, x1, y1 + roi_top)
                if bbox_aspect > 1.85:
                    continue
                if self.camera_sign_candidate_overlaps_current_traffic_light(
                    frame_bbox,
                    frame_width,
                    frame_height,
                    stamp_ns,
                    ...
                    continue
                if not self.camera_speed_limit_geometry_allows(component_fill_ratio, w, h):
                    continue
    ...
    def classify_camera_sign_crop(
        ...
        if turn_opposite_kind is not None:
            route_kind = self.camera_route_preferred_turn_kind(
                allowed_kinds=frozenset((best_template.kind, turn_opposite_kind))
            )
            if route_kind is not None:
                route_template = templates_by_kind.get(route_kind)
                route_score = scores_by_kind.get(route_kind, 0.0)
                if (
                    route_template is not None
                    and route_score >= self.camera_sign_min_score
                    and route_score >= best_score - CAMERA_SIGN_TURN_AMBIGUITY_MARGIN
                ):
                    return route_template, route_score
    ...
    def update_camera_instruction_from_frame(self, frame: np.ndarray, stamp_ns: int) -> None:
        detection = self.detect_camera_instruction(frame, stamp_ns)
        if detection is None:
            if self.camera_instruction_detection is not None and stamp_ns - self.last_camera_instruction_ns > self.camera_sign_timeout_ns:
                ...
                self.camera_instruction_stable_count = 0
                self.last_applied_instruction_key = None
                self.last_camera_instruction_log_name = None
                self.last_camera_sign_action_distance_guard_signature = None
    ...
    def camera_instruction_ready_for_action(self, detection: CameraInstructionDetection) -> bool:
        if not self.camera_scene_sign_context_allows_detection(detection):
            return False
        if not self.camera_sign_action_distance_allows(detection):
            return False
        ...
        if detection.maneuver is not None:
            route_camera_action = self.camera_route_maneuver_action_allows_detection(detection)
            return (
                detection.area_px >= self.camera_sign_action_min_area_px
                and (self.camera_instruction_center_allows_maneuver(detection) or route_camera_action)
                and self.maneuver_camera_route_context_allows_action(detection.maneuver)
            )
        return detection.area_px >= self.camera_sign_action_min_area_px

    def camera_sign_action_distance_allows(self, detection: CameraInstructionDetection) -> bool:
        action_max_distance_m = self.camera_sign_action_max_distance_for_detection(detection)
        if not self.camera_sign_scene_poses:
            signature = (detection.kind, detection.name, "no_scene_sign_context")
            if signature != self.last_camera_sign_action_distance_guard_signature:
                self.get_logger().info(
                    "camera sign action blocked until a matching 3D sign is close; "
                    f"detected={detection.kind} sign={detection.name} reason=no_scene_sign_context "
                    f"action_dist<={action_max_distance_m:.2f}m"
                )
                self.last_camera_sign_action_distance_guard_signature = signature
            return False

        close_scene_sign = self.nearest_facing_scene_sign_for_detection(
            detection,
            action_max_distance_m,
        )
        if close_scene_sign is not None:
            self.last_camera_sign_action_distance_guard_signature = None
            return True

        nearest_scene_sign = self.nearest_facing_scene_sign_for_detection(
            detection,
            self.sign_detection_max_distance_m,
        )
        if nearest_scene_sign is not None:
            sign_pose, sign_distance_m = nearest_scene_sign
            self.arm_preview_maneuver_from_scene_sign(
                detection,
                sign_pose,
                sign_distance_m,
                action_max_distance_m,
            )
            signature = (detection.kind, detection.name, sign_pose.name)
            if signature != self.last_camera_sign_action_distance_guard_signature:
                self.get_logger().info(
                    "camera sign seen but action is waiting until closer; "
                    f"detected={detection.kind} sign={detection.name} "
        ...
        signature = (detection.kind, detection.name, "no_matching_close_scene_sign")
        if signature != self.last_camera_sign_action_distance_guard_signature:
            self.get_logger().info(
                "camera sign action blocked; no matching facing 3D sign is close enough; "
                f"detected={detection.kind} sign={detection.name} "
                f"detection_dist<={self.sign_detection_max_distance_m:.2f}m "
                f"action_dist<={action_max_distance_m:.2f}m"
            )
            self.last_camera_sign_action_distance_guard_signature = signature
        return False

    def arm_preview_maneuver_from_scene_sign(
        self,
        detection: CameraInstructionDetection,
        sign_pose: SceneSignPose,
        sign_distance_m: float,
        action_max_distance_m: float,
    ) -> bool:
        if sign_distance_m > action_max_distance_m:
            return False
        if detection.maneuver not in {"left", "right"}:
            return False
        if detection.area_px < self.camera_sign_action_min_area_px:
            return False
        ...
        if self.track.pending_maneuver == PARKING_MANEUVER:
            return False
        if self.track.pending_maneuver == detection.maneuver:
            return True
        if self.track.pending_maneuver in {"left", "right"} and self.track.pending_maneuver != detection.maneuver:
            return False
        if not self.camera_instruction_center_allows_maneuver(detection):
            return False
        if self.track.distance_to_next_maneuver_branch(float("inf")) is None:
            return False

        self.track.set_pending_maneuver(detection.maneuver, sign_pose.name)
        self.get_logger().info(
            f"armed preview maneuver '{detection.maneuver}' from camera sign {detection.name}; "
            f"scene_sign={sign_pose.name} sign_dist={sign_distance_m:.3f}m "
            f"action_dist<={action_max_distance_m:.2f}m "
    ...
    def camera_sign_action_max_distance_for_detection(self, detection: CameraInstructionDetection) -> float:
        if detection.kind == "parking_sign":
            return self.parking_sign_action_max_distance_m
        return self.sign_action_max_distance_m
    ...
    def parking_camera_route_context_allows_arm(self) -> bool:
        ...
        if segment.inner_route:
            if parking_branch_distance is not None:
                return True
            return self.nearest_facing_scene_sign(
                "parking_sign",
                self.parking_sign_action_max_distance_m,
            ) is not None
        if not self.parking_scene_sign_faces_car():
            return False
    ...
    def camera_route_preferred_turn_kind(self, allowed_kinds: frozenset[str] | None = None) -> str | None:
        available_actions = self.track.maneuver_actions_at_next_branch(self.sign_detection_max_distance_m)
        preferred_kinds: list[str] = []
        for action, kind in CAMERA_SIGN_MANEUVER_KIND_BY_ACTION.items():
        ...
        if len(preferred_kinds) == 1:
            return preferred_kinds[0]
        return None

    def camera_route_maneuver_action_allows_detection(self, detection: CameraInstructionDetection) -> bool:
        if detection.maneuver not in {"left", "right"}:
            return False
        if detection.area_px < self.camera_sign_action_min_area_px:
            return False
        available_actions = self.track.maneuver_actions_at_next_branch(self.sign_detection_max_distance_m)
        return detection.maneuver in available_actions
    ...
    def camera_pending_maneuver_conflict_blocks(self, visible: CameraInstructionDetection) -> bool:
        ...
        if pending == "straight" and visible.maneuver in {"left", "right"}:
            self.get_logger().info(
                "camera turn maneuver overrides queued straight command; "
                f"pending_source={source_name} "
                f"new='{visible.maneuver}' sign={visible.name} "
                f"next_branch={self.track.next_maneuver_branch_distance_text()}"
            )
            return False
    ...
    def apply_instruction(self, visible: CameraInstructionDetection) -> bool:
        ...
        if visible.kind == "parking_sign":
            ...
            self.track.set_pending_maneuver(PARKING_MANEUVER, visible.name)
            parking_wait_context = (
                "staying on the middle route until the next parking branch"
                if self.track.current_segment.inner_route
                else f"continuing on the outer route until the {self.track.parking_entry_label}"
            )
            self.get_logger().info(
                f"parking route armed from camera sign {visible.name}; "
                f"{parking_wait_context}."
            )
            return True
        ...
        if visible.maneuver is not None:
            ...
            nearby = self.track.rejoin_nearby_maneuver(
                ...
                SIGN_LOCAL_MANEUVER_MAX_ROUTE_DISTANCE_M,
            )
            ...
            if scene_nearby is not None:
                ...
                self.route_alignment_suppressed_until_ns = max(
                    self.route_alignment_suppressed_until_ns,
                    now_ns + int(SIGN_LOCAL_MANEUVER_SYNC_SUPPRESS_S * 1e9),
                )
                self.last_segment_name = self.track.current_segment.name
                self.publish_route_trace(now_ns, force=True)
                self.get_logger().info(
                    ...
                    f"search_radius={maneuver_search_distance_m:.2f}m "
                    f"delta={math.degrees(delta):+.1f}deg "
                    f"sync_suppressed={SIGN_LOCAL_MANEUVER_SYNC_SUPPRESS_S:.1f}s"
                )
                return True
            ...
            if scene_branch_distance is not None:
                if scene_sign is not None:
                    sign_pose, sign_distance_m = scene_sign
                    source_name = sign_pose.name
                    sign_text = f"scene_sign={sign_pose.name} sign_dist={sign_distance_m:.3f}m "
                else:
                    source_name = visible.name
                    sign_text = "scene_sign=route_camera_context "
                self.track.set_pending_maneuver(visible.maneuver, source_name)
                self.get_logger().info(
            ...
            if current_route_matches:
                ...
                self.get_logger().info(
                    ...
                    f"matches current route after no inner branch was available; "
                    f"continuing segment={self.track.current_segment.name} delta={delta_text}"
                )
                return True
    ...
    def reset_initial_pose_if_needed(self, sample: MotionSample) -> bool:
        ...
        target_x, target_y, target_yaw = self.initial_pose_reset_target()
        position_error = math.hypot(sample.x - target_x, sample.y - target_y)
        ...
        state = self.make_entity_state(self.entity_name, target_x, target_y, self.z, target_yaw)
        request = SetEntityState.Request()
        request.state = state
        ...
        self.get_logger().info(
            ...
            f"target=({target_x:+.3f},{target_y:+.3f}) yaw={target_yaw:+.3f} "
            f"error={position_error:.3f}m/{yaw_error:.3f}rad "
            f"segment={self.track.current_segment.name}"
        )
        return True

    def initial_pose_reset_target(self) -> tuple[float, float, float]:
        if self.initial_pose_override_enabled:
            return self.initial_pose_x_m, self.initial_pose_y_m, self.initial_pose_yaw_rad
        route_x, route_y, route_yaw = self.track.sample()
        return route_x, route_y, normalize_angle(route_yaw + self.yaw_offset)
    ...
    def sync_route_progress_to_actual(self, sample: MotionSample) -> bool:
        if (
            ...
            return False
        if sample.stamp_ns < self.route_alignment_suppressed_until_ns:
            return False
