...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.16,
        help="Minimum rendered sign plate size as a fraction of image width.",
    )
    parser.add_argument(
        "--max-plate-fraction",
        type=float,
        default=0.42,
        help="Maximum rendered sign plate size as a fraction of image width.",
    )
    parser.add_argument(
        "--negative-hard-ratio",
        type=float,
        default=0.0,
        help="Fraction of negative images with sign-like distractors such as blank sign backs.",
    )
...
def render_one(
    ...
    rng: random.Random,
    min_object_px: int,
    min_plate_fraction: float,
    max_plate_fraction: float,
    ...
    for _ in range(200):
        ...
        plate_size = rng.uniform(width * min_plate_fraction, width * max_plate_fraction)
...
def write_dataset_metadata(
    ...
    config = {
        ...
        "min_plate_fraction": args.min_plate_fraction,
        "max_plate_fraction": args.max_plate_fraction,
...
def draw_blank_sign_back_distractor(image: np.ndarray, width: int, height: int, rng: random.Random) -> None:
    plate_size = rng.uniform(width * 0.05, width * 0.34)
    focal = rng.uniform(330.0, 820.0)
    yaw = rng.uniform(-72.0, 72.0)
    pitch = rng.uniform(-42.0, 36.0)
    roll = rng.uniform(-15.0, 15.0)
    quad = project_plate_quad(plate_size, yaw, pitch, roll, focal)

    min_xy = quad.min(axis=0)
    max_xy = quad.max(axis=0)
    margin = int(max(8, min(width, height) * 0.025))
    center_min_x = margin - min_xy[0]
    center_max_x = width - margin - max_xy[0]
    center_min_y = margin - min_xy[1]
    center_max_y = height - margin - max_xy[1]
    ...
    dst_quad = quad + center
    draw_pole_and_shadow(image, dst_quad, rng)

    quad_int = np.round(dst_quad).astype(np.int32)
    shade = rng.randint(220, 250)
    cv2.fillConvexPoly(image, quad_int, (shade, shade, shade), cv2.LINE_AA)
    border = max(1, int(plate_size * rng.uniform(0.006, 0.018)))
    cv2.polylines(image, [quad_int], True, (190, 190, 190), border, cv2.LINE_AA)

    if rng.random() < 0.45:
        support = np.mean([dst_quad[2], dst_quad[3]], axis=0)
        tip = support + np.array([rng.uniform(-20, 20), rng.uniform(20, 70)], dtype=np.float32)
        cv2.line(
            image,
...
def draw_traffic_light_distractor(image: np.ndarray, width: int, height: int, rng: random.Random) -> None:
    scale = rng.uniform(0.45, 1.25)
    box_w = int(width * rng.uniform(0.025, 0.055) * scale)
    box_h = int(box_w * rng.uniform(2.1, 2.9))
    x = rng.randint(0, max(1, width - box_w - 1))
    y = rng.randint(int(height * 0.12), max(int(height * 0.13), height - box_h - 35))
    pole_len = rng.randint(35, 160)
    pole_bottom = min(height - 1, y + box_h + pole_len)
    pole_x = x + box_w // 2 + rng.randint(-8, 8)

    cv2.line(image, (pole_x, y + box_h), (pole_x, pole_bottom), (35, 35, 35), max(2, box_w // 7), cv2.LINE_AA)
    cv2.rectangle(image, (x, y), (x + box_w, y + box_h), (18, 18, 18), -1, cv2.LINE_AA)
    radius = max(2, box_w // 5)
    for idx, color in enumerate(((35, 35, 210), (45, 190, 220), (45, 180, 55))):
        cy = y + int(box_h * (0.22 + idx * 0.28))
        cv2.circle(image, (x + box_w // 2, cy), radius, color, -1, cv2.LINE_AA)


def generate_negative_image(width: int, height: int, rng: random.Random, hard_ratio: float) -> np.ndarray:
    image = make_background(width, height, rng)
    if rng.random() < hard_ratio:
        for _ in range(rng.randint(1, 3)):
            if rng.random() < 0.75:
                draw_blank_sign_back_distractor(image, width, height, rng)
            else:
                draw_traffic_light_distractor(image, width, height, rng)
    if rng.random() < 0.4:
...
def main() -> None:
    ...
    if not 0.01 <= args.min_plate_fraction <= args.max_plate_fraction <= 0.95:
        raise ValueError("--min-plate-fraction and --max-plate-fraction are invalid")
    if not 0.0 <= args.negative_hard_ratio <= 1.0:
        raise ValueError("--negative-hard-ratio must be between 0.0 and 1.0")
    ...
    preview_per_class = max(1, 16 // len(selected_signs))
    ...
    for index in range(args.negative_samples):
        ...
        image_path = args.output_dir / "images" / split / f"{stem}.{args.format}"
        label_path = args.output_dir / "labels" / split / f"{stem}.txt"
        image = generate_negative_image(
            args.image_width,
            args.image_height,
            rng,
            hard_ratio=args.negative_hard_ratio,
        )
        cv2.imwrite(str(image_path), image)
