...
class RobotSimulator(Node):
    def __init__(self) -> None:
        ...
        self.last_motion_stopped: bool | None = None
        self.elapsed = 0.0
    ...
    def step(self) -> None:
        ...
        cmd_age = time.monotonic() - self.last_cmd_time
        timed_out = cmd_age > 0.8
        if timed_out:
            linear_cmd = 0.0
            angular_cmd = 0.0
        else:
        ...
        self.log_motion(linear_cmd, angular_cmd, cmd_age, timed_out)
    ...
    def log_motion(self, linear_cmd: float, angular_cmd: float, cmd_age: float, timed_out: bool) -> None:
        if timed_out and not self.cmd_timeout_active:
            self.get_logger().warn(f"No {self.cmd_topic} for {cmd_age:.2f}s; applying watchdog stop.")
        elif self.cmd_timeout_active and not timed_out:
            self.get_logger().info(f"{self.cmd_topic} recovered after watchdog stop.")
        self.cmd_timeout_active = timed_out

        now = time.monotonic()
        linear_stopped = abs(linear_cmd) < 1e-6
        stop_changed = linear_stopped != self.last_motion_stopped
        if not stop_changed and now - self.last_motion_log_time < 1.0:
            return

        source = "WATCHDOG" if timed_out else "CMD_VEL"
        message = (
            f"motion source={source}; cmd_age={cmd_age:.2f}s; "
            f"applied linear.x={linear_cmd:+.2f} angular.z={angular_cmd:+.2f}"
        )
        if linear_stopped:
        ...
        self.last_motion_stopped = linear_stopped
